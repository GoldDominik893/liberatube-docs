...
In the config.php you will find these 4 variables:
...
`$InvVIServerArray` - The Invidious instance list used for fetching Video Information, such as the title, description and the direct video URL's.
                    - You can configure a list of instances that liberatube can use for a failover system, if one of the instances returns a foul response it will try the next one on the list.
